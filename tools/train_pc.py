...
from lib.core.loss_pc import get_loss
from lib.core.function_pc import train
from lib.core.function_pc import validate
...
from gradoptim.pcgrad2 import PCGrad
from gradoptim.gradvac_amp import GradVacAMP
from gradoptim.pcgrad_amp import PCGradAMP
...
def main():
    ...
    optimizer = PCGradAMP(3, optimizer, scaler, reduction='sum')
    ...
    Encoder_para_idx = [str(i) for i in range(0, 9)]
    Det_Head_para_idx = [str(i) for i in range(17, 25)]
    Da_Seg_Head_para_idx = [str(i) for i in range(25, 34)]
    # Ll_Seg_Head_para_idx = [str(i) for i in range(34, 42)]
    Ll_Seg_Head_para_idx = [str(i) for i in range(34, 47)]

    # load pth file
    model_state= model.state_dict()
    pth_file= torch.load('../weights/End-to-end.pth')
    pth_state= pth_file['state_dict']
    checkpoint_dict = {k: v for k, v in pth_state.items() if k.split(".")[1] in Encoder_para_idx}
    model_state.update(checkpoint_dict)
    model.load_state_dict(model_state)

    lf = lambda x: ((1 + math.cos(x * math.pi / cfg.TRAIN.END_EPOCH)) / 2) * \
                   (1 - cfg.TRAIN.LRF) + cfg.TRAIN.LRF  # cosine
    lr_scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer.optimizer, lr_lambda=lf)
