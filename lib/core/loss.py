...
class MultiHeadLoss(nn.Module):
    ...
    def __init__(self, losses, cfg, lambdas=None):
        ...
        if not lambdas:
            lambdas = [1.0 for _ in range(len(losses)-1 + 3)]
    ...
    def _forward_impl(self, predictions, targets, shapes, model):
        ...
        cp, cn = smooth_BCE(eps=0.0)  # 0.95  0.05

        BCEcls, BCEobj, BCEseg, Focalll = self.losses
        ...
        drive_area_seg_predicts = predictions[1].reshape(-1)
        drive_area_seg_targets = targets[1].reshape(-1)
        lseg_da = BCEseg(drive_area_seg_predicts, drive_area_seg_targets)

        lane_line_seg_predicts = predictions[2].reshape(-1)
        lane_line_seg_targets = targets[2].reshape(-1)
        # lseg_ll = BCEseg(lane_line_seg_predicts, lane_line_seg_targets)
        lseg_ll= Focalll(lane_line_seg_predicts, lane_line_seg_targets)
...
def get_loss(cfg, device):
    ...
    BCEseg_ll = nn.BCEWithLogitsLoss(pos_weight=torch.Tensor([cfg.LOSS.SEG_POS_WEIGHT])).to(device)
    # Focal loss
    gamma = cfg.LOSS.FL_GAMMA  # focal loss gamma
    if gamma > 0:
        BCEcls, BCEobj = FocalLoss(BCEcls, gamma), FocalLoss(BCEobj, gamma)

    focal_ll= FocalLoss(BCEseg_ll)
    loss_list = [BCEcls, BCEobj, BCEseg, focal_ll]
